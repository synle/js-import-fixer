...
describe("coreUtils.parseRawImportLines", () => {
  test("with transform relative", async () => {
    const actual = coreUtils.parseRawImportLines("MySimpleLibrary as MyAliasName");
    expect(actual).toBe("MySimpleLibrary");
  });

  test("without any transformation", async () => {
    const actual = coreUtils.parseRawImportLines("MySimpleLibrary");
    expect(actual).toBe("MySimpleLibrary");
  });
});
